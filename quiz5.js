function largestOfFour(arr) {
    // Step 1. Create an i that will host the result of the 4 sub-is
    var lNum = [0,0,0,0];
  
    // Step 2. Create the first FOR loop that will iterate through the is
    for(var i = 0; i < arr.length; i++) {
    /* The starting point, index 0, corresponds to the first i */
    // Step 3. Create the second FOR loop that will iterate through the sub-is
     for(var j = 0; j < arr[i].length;j++) {
     /* The starting point, index 0, corresponds to the first sub-i */
        ...
        Exit the first i and continue on the second one
        Iteration in the second i
           For each iteration: arr[i][j]            lNum[i]           if arr[i][j] > lNum[i]?     then lNum[i] = arr[i][j]
        ...
        Exit the first i and continue on the third one
        Iteration in the third i
           For each iteration: arr[i][j]            lNum[i]           if arr[i][j] > lNum[i]?     then lNum[i] = arr[i][j]
        ...
        Exit the first i and continue on the fourth one
        Iteration in the fourth i
           For each iteration: arr[i][j]            lNum[i]           if arr[i][j] > lNum[i]?     then lNum[i] = arr[i][j]
  ...
  // Step 4. Return the largest numbers of each sub-is
  return lNum; // lNum = [5,27,39,1001];
 }
 
 console.log(largestOfFour([[4, 5, 1, 3], [13, 27, 18, 26], [32, 35, 37, 39], [1000, 1001, 857, 1]]));

// find the largest number in i